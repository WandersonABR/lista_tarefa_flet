...
class ToDo:
    def __init__(self, page: ft.Page):
        ...
        self.task = ''
        self.view = 'all'
        self.db_execute('CREATE TABLE IF NOT EXISTS task(name, status)')
    ...
    def checked(self, e):
        ...
        if is_checked:
            self.db_execute('UPDATE task SET status = "complete" WHERE name = ?', params=[label])
        else:
            self.db_execute('UPDATE task SET status = "incomplete" WHERE name = ?', params=[label])
            
        if self.view == 'all':
            self.results = self.db_execute('SELECT * FROM task')
        else:
            self.results = self.db_execute('SELECT * FROM task WHERE status = ?', params=self.view)
    ...
    def add(self, e, input_task):
        ...
        if name:
            self.db_execute(query = 'INSERT INTO task VALUES(?,?)', params=[name, status])
            input_task.value = ''
            self.results = self.db_execute('SELECT * FROM task')
            self.update_task_list()
